...
| `$wgGTagRequireCookieConsent` | `true` | When [CookieConsent][6] is loaded, GTag waits for statistics consent before loading Google Analytics. Set to `false` to load Google without waiting. Ignored if CookieConsent is not loaded. |
...
Set `$wgGTagRequireCookieConsent = false` if you do not want GTag to wait for consent.
